...
class Access_Model_DbTable_User extends Zend_Db_Table_Abstract {
	...
	public function getUsers($limit, $offset, $id_User, $id_Role, $active, $username) {
		...
		$username != -1 ? $select->where("username LIKE ?", $username) : null;
//		echo $select;
	...
	public function editUser($id, $username, $password, $active, $id_Role, $id_User, $encodedpass) {

		$password = $encodedpass ? $password : SHA1($password . "sodiumchloride");
		$data = array(
		    'username' => $username,
		    'password' => $password,
		    'active' => $active,
