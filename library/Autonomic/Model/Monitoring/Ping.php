...
define('NET_PING_CANT_LOCATE_PING_BINARY_MSG',
 ...
 * @access   public
 */
class Autonomic_Model_Monitoring_Ping {
     ...
     * @access private
     */
    function Autonomic_Model_Monitoring_Ping( $ping_path, $sysname ) {
    ...
    function factory() {
        ...
        $sysname = Autonomic_Model_Monitoring_Ping::_setSystemName();

        if( ($ping_path = Autonomic_Model_Monitoring_Ping::_setPingPath($sysname)) == NET_PING_CANT_LOCATE_PING_BINARY ) {
            return PEAR::raiseError(NET_PING_CANT_LOCATE_PING_BINARY_MSG,
                    NET_PING_CANT_LOCATE_PING_BINARY);
        } else {
            return new Autonomic_Model_Monitoring_Ping($ping_path, $sysname);
        }
    }
    ...
    function setArgs( $args ) {
        if( !is_array($args) ) {
            return PEAR::raiseError(NET_PING_INVALID_ARGUMENTS_MSG,
                    NET_PING_INVALID_ARGUMENTS);
        }
    ...
    function ping( $host ) {
        ...
        if( count($this->_result) == 0 ) {
            return PEAR::raiseError(NET_PING_HOST_NOT_FOUND_MSG,
                    NET_PING_HOST_NOT_FOUND);
        } else {
     ...
     * with prefixing a "@" sign to the function call: @Autonomic_Model_Monitoring_Ping::ping(..);
     *
...
class Net_Ping_Result {
    ...
    var $_ttl;
    /**
     * The raw Autonomic_Model_Monitoring_Ping::result
     *
     * @var array
     * @access private
     */
    var $_raw_data = array();
    /**
     * The Autonomic_Model_Monitoring_Ping::_sysname
     *
     * @var int
     * @access private
     */
    ...
    function _parseResultDetailIcmpSequence( $upper ) {
        ...
        for( $i = 1; $i < count($upper); $i++ ) {
            ...
            if( preg_match('/=\s*([\d+\.]+)\s*ms/i', $upper[$i], $matches) ) {
                ...
                if( preg_match('/icmp_seq\s*=\s*([\d+]+)/i', $upper[$i],
                                $matches) ) {
    ...
    function _parseResultDetailRoundTrip( $lower ) {
        ...
        for( $i = (count($lower) - 1); $i >= 0; $i-- ) {
            if( preg_match('|(' . $p1 . ')[^0-9]+(' . $p2 . ')|i', $lower[$i],
                            $matches) ) {
                break;
            }
        }
        ...
        for( $i = (count($lower) - 1); $i >= 0; $i-- ) {
            if( preg_match('/min.*max/i', $lower[$i]) ) {
                if( preg_match('/' . $p3 . $p3 . $p3 . '/i', $lower[$i],
                                $matches) ) {
...
/* class Net_Ping_Result */
//echo microtime(true) . "\r\n";
//echo Autonomic_Model_Monitoring_Ping::run("google.co.uk") . "\r\n";
//echo microtime(true) . "\r\n";
//echo microtime(true) . "\r\n";
//echo Autonomic_Model_Monitoring_Ping::run("icheev.com") . "\r\n";
//echo microtime(true) . "\r\n";
?>
